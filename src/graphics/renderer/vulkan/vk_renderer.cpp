...
namespace mip {
  ...
  //INTERFACE====================================================================================================
  std::shared_ptr<IMesh> VulkanRenderer::createMesh(const MeshData& data) {
    auto tempMesh = std::make_shared<VulkanMesh>();
    if(!tempMesh->init(data, m_physDev, m_logDev, m_cmdPool, m_graphQ)) return nullptr;
  ...
  std::shared_ptr<IMaterial> VulkanRenderer::createMaterial(const std::string& vertShaderPath, const std::string& fragShaderPath) {
    auto tempMaterial = std::make_shared<VulkanMaterial>();
  ...
  bool VulkanRenderer::beginFrame(const CameraInfo& camera) {
    ...
    m_imagesInFlight[m_curImgIndex] = *m_inFlightFences[m_curFrame];
    m_logDev.resetFences(*m_inFlightFences[m_curFrame]);
    
    m_curCmdBuf = &m_cmdBufs[m_curFrame];
    m_curCmdBuf->reset();
    m_curCmdBuf->begin({});
    
    CameraData cameraData{.view = camera.view, .proj = camera.projection};
    ...
    vk::RenderingAttachmentInfo clrAttachmentInfo{
      .imageView = m_imgViews[m_curImgIndex],
  ...
  bool VulkanRenderer::submit(std::shared_ptr<IMesh> mesh, std::shared_ptr<IMaterial> material, const RenderInfo& info) {
    ...
    m_curCmdBuf->drawIndexed(vkMesh->getIndexCount(), 1, 0, 0, 0);
    
    
    return true;
  }
  
  // bool VulkanRenderer::submitAnimated(std::shared_ptr<IMesh> mesh, std::shared_ptr<IMaterial> material, const glm::mat4& transform, const std::vector<glm::mat4>& boneTransforms) {
  //   auto vkMesh = std::static_pointer_cast<VulkanMesh>(mesh);
  //   auto vkMaterial = std::static_pointer_cast<VulkanMaterial>(material);
  ...
  //   BoneData boneData{};
  //   memcpy(boneData.bones, boneTransforms.data(), boneTransforms.size() * sizeof(glm::mat4));
  ...
  bool VulkanRenderer::endFrame() {
    ...
    const vk::SubmitInfo submitInfo{
      ...
      .pSignalSemaphores = &*m_renderFinishedSems[m_curImgIndex]
    };
    m_graphQ.submit(submitInfo, *m_inFlightFences[m_curFrame]);
    
    const vk::PresentInfoKHR presInfo{
      .waitSemaphoreCount = 1,
      .pWaitSemaphores = &*m_renderFinishedSems[m_curImgIndex],
      .swapchainCount = 1,
      .pSwapchains = &*m_sc.getSC(),
      .pImageIndices = &m_curImgIndex
    };
    auto res = m_presQ.presentKHR(presInfo);
  ...
  bool VulkanRenderer::createInstance() {
    ...
    std::vector<vk::LayerProperties> layerProps = m_ctx.enumerateInstanceLayerProperties();
    // {
    //   auto res = ;
    //   if(!res) {
    ...
    std::vector<vk::ExtensionProperties> extensionProps = m_ctx.enumerateInstanceExtensionProperties();
    // {
    ...
    if(res) {
      ...
      Logger::error("Failed to create instance: {}", vk::to_string(res.error()));
      return false;
    }
  ...
  bool VulkanRenderer::setupDM() {
    ...
    };
    {
      ...
      if(!res) {
        Logger::error("Failed to create debug messenger: {}", vk::to_string(res.error()));
        return false;
      }
    ...
    if(res) {
      ...
      const auto devIter = std::ranges::find_if(devs,
        [&](auto const& dev) {
          ...
          std::vector<vk::ExtensionProperties> extensions = dev.enumerateDeviceExtensionProperties();
          // {
      ...
      Logger::error("Failed to enumerate physical devices: {}", vk::to_string(res.error()));
      return false;
    }
  }
  ...
  bool VulkanRenderer::createLogDev() {
    ...
    m_graphQI = static_cast<uint32_t>(std::distance(qfProps.begin(), graphQFP));
    {
      ...
      m_presQI = res ? m_graphQI : static_cast<uint32_t>(qfProps.size());
    }
    ...
    
    {
      ...
      if(!res) {
        Logger::error("Failed to get graph queue: {}", vk::to_string(res.error()));
        return false;
      }
      m_graphQ = std::move(*res);
    }
    {
  ...
  bool VulkanRenderer::initImGui() {
    ...
    if(!res)
      return false;
    m_imguiDescPool = std::move(res.value());
    ...
    VkFormat clrFormat = static_cast<VkFormat>(m_sc.getFormat());
    VkPipelineRenderingCreateInfoKHR pipelineInfo{
      .sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR,
      .colorAttachmentCount = 1,
      .pColorAttachmentFormats = &clrFormat
    };
    ImGui_ImplVulkan_InitInfo initInfo{
      .Instance = *m_inst,
      .PhysicalDevice = *m_physDev,
      .Device = *m_logDev,
      .QueueFamily = m_graphQI,
      .Queue = *m_graphQ,
  ...
  void VulkanRenderer::renderImGui() {
    ...
    if(drawData) {
      ImGui_ImplVulkan_RenderDrawData(drawData, *m_cmdBufs[m_curFrame]);
    }
  }
